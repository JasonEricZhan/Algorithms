...
void dp(int length,float **cost,float **height,int **path)
{
   
  
    //initialize,(0,0) and (0,1~n) and (1~n,0),
    *(*(cost+0)+0)=0+*(*(height+0)+0);
    *(*(path+0)+0)=0;   //because it's the starting point so is 0
    //(1,0)  is legal 
    *(*(cost+0)+1)=*(*(cost+0)+0)+diff(0,1,0,0,height)+*(*(height+0)+1);
    *(*(path+0)+1)=1;  //because the only way to go to (0,1) is from (0,0) and move horizon direction
    //(0,1) is illegal 
    *(*(cost+1)+0)=INT_MAX;
    //other are illegal
